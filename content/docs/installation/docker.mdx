...
- Creates a container named `dicedb-1`
...
```bash
# Using docker exec
docker exec -it dicedb-1 dicedb-cli
...
```bash
docker stop dicedb-1
```

### Start the Container

```bash
docker start dicedb-1
```

### View Logs

```bash
docker logs dicedb-1

# Follow logs in real-time
docker logs -f dicedb-1
```
