...
1. When a key is deleted using the `DEL` command, it is removed from both memory and RocksDB if it exists in either location
2. When `FLUSHDB` or `FLUSHALL` is executed, the corresponding RocksDB instances are also cleared
3. When the DiceDB server starts, all RocksDB instances used by the Spill module are initialized fresh, ensuring that any previously spilled data is removed
